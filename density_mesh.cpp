//
// Calculates Clound-in-Cell (CIC) density and momentum fields
//
				...
				)
{
  ...
  for(int ix=0; ix<nc; ++ix) {
	...
	// n(x) to density constrast delta(x)
				...
				)
{
  ...
  for(vector<ParticleData>::const_iterator p= v.begin(); p != v.end(); ++p) {
    ...
    float u= 0.01f*p->v[2];                 // in Units of 100km/s
  ...
  for(int ix=0; ix<nc; ++ix) {
    for(int iy=0; iy<nc; ++iy) {
      for(int iz=0; iz<nc; ++iz) {
	int index= ncz*(nc*ix + iy) + iz;
  	pmesh[index]= pmesh[index]*nbar_inv; 
      }
    }
  }
}
